...
export function DeleteAccountButton() {
  ...
  if (!confirming) {
    return (
      <button onClick={() => setConfirming(true)} className="text-sm text-danger hover:text-red-700 transition-colors">
        {t("button")}
      </button>
    );
  }
  ...
  return (
    <div className="bg-danger-light border border-danger-border rounded-xl p-4 space-y-3">
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="text-sm font-semibold text-danger">{t("permanentTitle")}</p>
          <p className="text-xs text-text-muted mt-0.5">{t("permanentDesc")}</p>
        </div>
      </div>
