  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-surface rounded-2xl border border-border shadow-sm overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-border flex items-center gap-5">
        ...
        <div>
          <p className="text-sm font-medium text-text">{t("photoLabel")}</p>
          <p className="text-xs text-text-faint mt-0.5">{t("photoHint")}</p>
        </div>
      </div>

      <div className="px-6 py-5 border-b border-border">
        <h2 className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-4">{t("requiredInfo")}</h2>
        <div className="grid sm:grid-cols-2 gap-4">
          <Input label={t("firstNameLabel")} required placeholder={t("firstNamePlaceholder")} error={errors.first_name?.message} {...register("first_name")} />
          <Input label={t("lastNameLabel")} required placeholder={t("lastNamePlaceholder")} error={errors.last_name?.message} {...register("last_name")} />
        </div>
      </div>

      <div className="px-6 py-5 space-y-3">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-xs font-semibold text-text-muted uppercase tracking-wide">{t("additionalInfo")}</h2>
          <span className="text-xs text-text-faint">{t("visibilityHint")}</span>
        </div>
        <ToggleRow label={t("genderLabel")} toggleName="show_gender" error={errors.gender?.message} control={control} publicLabel={t("public")}>
        ...
        <ToggleRow label={t("birthDateLabel")} toggleName="show_birth_date" error={errors.birth_date?.message} control={control} publicLabel={t("public")}>
          <input {...register("birth_date")} type="date" className={fieldCls(!!errors.birth_date)} />
        </ToggleRow>
        <ToggleRow label={t("phoneLabel")} toggleName="show_phone" error={errors.phone?.message} control={control} publicLabel={t("public")}>
          <input {...register("phone")} type="tel" placeholder={t("phonePlaceholder")} className={fieldCls(!!errors.phone)} />
        </ToggleRow>
        <ToggleRow label={t("addressLabel")} toggleName="show_address" error={errors.address?.message} control={control} publicLabel={t("public")}>
          <input {...register("address")} type="text" placeholder={t("addressPlaceholder")} className={fieldCls(!!errors.address)} />
        </ToggleRow>
        <ToggleRow label={t("emailLabel")} toggleName="show_email" error={errors.email?.message} control={control} publicLabel={t("public")}>
          <input {...register("email")} type="email" placeholder={t("emailPlaceholder")} className={fieldCls(!!errors.email)} />
        </ToggleRow>
...
function ToggleRow({ label, toggleName, error, control, children, publicLabel }: {
  label: string;
  ...
  return (
    <div className="p-3 rounded-xl border border-border hover:border-primary-muted transition-colors">
      <div className="flex items-center gap-3">
        ...
        <Controller name={toggleName} control={control}
          render={({ field }) => (
            <label className="flex flex-col items-center gap-1 cursor-pointer shrink-0 select-none">
              ...
              <span className="text-xs text-text-faint whitespace-nowrap">{publicLabel}</span>
            </label>
          )}
        />
      </div>
