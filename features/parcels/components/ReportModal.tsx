...
export function ReportModal({ parcelId, onClose }: { parcelId: string; onClose: () => void }) {
  ...
  function handleSubmit() {
    if (!reason) { setError(t("reason")); return; }
    setError(null);
    startTransition(async () => {
      const res = await reportParcel(parcelId, reason, details);
      if (res?.error) { setError(res.error); } else { setSuccess(true); }
    });
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        ...
        {success ? (
          <div className="text-center py-4">
            ...
            <p className="font-medium text-text mb-1">{t("successTitle")}</p>
            <p className="text-sm text-text-muted mb-4">{t("successDesc")}</p>
          ...
          <>
              ...
              {REASONS.map(r => (
                <label key={r.value} className="flex items-center gap-3 cursor-pointer group">
                  <input type="radio" name="reason" value={r.value} checked={reason === r.value}
                    onChange={() => setReason(r.value)} className="accent-primary" />
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-text mb-1.5">
                {t("details")} <span className="text-text-faint">{t("detailsOptional")}</span>
              </label>
